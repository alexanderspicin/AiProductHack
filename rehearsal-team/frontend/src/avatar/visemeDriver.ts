...
export class VisemeDriver {
  ...
  handleAvatarEvent(event: AvatarEvent) {
    if (event.type === "viseme") {
      if (event.utterance_id !== this.currentUtteranceId || this.playbackDone) return;
      ...
      if (event.utterance_id <= this.currentUtteranceId) return;
      ...
      if (event.utterance_id === this.currentUtteranceId) {
        ...
        this.currentWeights = {};
        this.compositor.set("viseme", {});
        this.presence.mouthOpen = 0;
      }
    }
  }
