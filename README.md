...
Основной вариант по архитектуре исходной презентации: [rehearsal-team](rehearsal-team/README.md). Он использует настоящий бэкенд команды из соседнего `backend/app`: Pipecat, серверные Silero VAD и Smart Turn, Whisper, OpenAI, Inworld и локальный 3D-персонаж.

Обе версии содержат кабинеты участника, методиста и администратора, сценарии, диалоги и отчёты. [Навигация и запуск](FINAL_VERSION.md).

- [Запуск варианта на бэкенде команды](rehearsal-team/README.md), [проверки и соответствие слайдам](rehearsal-team/DEFENSE.md).
- [Отдельная API-версия: Cartesia + Tavus/Anam](rehearsal/README.md).
...
Новые версии добавлены рядом с исходным проектом. Старые `backend/`, `frontend/`, Docker-файлы и описание ниже сохранены. Команды выполняются из папки выбранной версии; инструкции ниже относятся к первоначальному голосовому прототипу.
