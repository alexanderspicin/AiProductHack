...
from app.avatar import protocol
from app.avatar.viseme_analysis import LipsyncAnalyzer
from app.avatar.viseme_text import VisemeTimeline, jaw_openness
...
class VisemeProcessor(FrameProcessor):
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, TTSStartedFrame):
            ...
            if self._first_audio_clock_s is None:
                self._first_audio_clock_s = self._clock_s
    ...
    async def _tick_loop(self, direction: FrameDirection) -> None:
        ...
        await self.push_frame(
            RTVIServerMessageFrame(data=protocol.playback_end(utterance_id)), direction
        )
