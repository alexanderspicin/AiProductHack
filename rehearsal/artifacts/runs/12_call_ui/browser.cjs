...
async function main() {
  ...
  try {
    ...
    for (const [width, height] of [[1440,900],[1280,720],[1024,768],[390,844],[667,375],[320,640],[390,420],[320,360]]) {
      ...
      measures.pass &&= measures.finishButton.y >= 0 && measures.finishButton.y + measures.finishButton.height <= height
      await page.keyboard.press('Escape')
    ...
    result.passed = result.viewports.every(v => v.pass && v.taskVisible && v.taskClosed && v.optionsClosed && v.optionsClickAway && v.draftRestored) && result.text_viewports.every(v => v.pass) && !result.browser_errors.length && !result.unexpected_writes.length
    await fs.writeFile(path.join(output, 'browser.json'), JSON.stringify(result, null, 2) + '\n')
